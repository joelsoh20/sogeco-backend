...
/**
 * Diffusion des evenements applicatifs vers les canaux STOMP.
 *
 * Aucun service metier ne connait le WebSocket : ils publient des
 * evenements, ce composant les traduit en messages. On peut donc
 * changer de transport sans toucher au metier.
 */
@Slf4j
@Component
...
public class TrackingBroadcaster {
    ...
    public static final String TOPIC_POSITION_ONE    = "/topic/vehicle-positions/%d";
    public static final String TOPIC_ALERTS          = "/topic/alerts";
    public static final String TOPIC_ALERTS_CRITICAL = "/topic/alerts/critical";
    public static final String TOPIC_MISSION         = "/topic/missions/%d/progress";
    ...
    public void onPosition(PositionReceivedEvent event) {
        try {
            ...
            messaging.convertAndSend(
                    TOPIC_POSITION_ONE.formatted(event.position().vehicleId()), event.position());

            if (event.position().missionId() != null) {
                messaging.convertAndSend(
                        TOPIC_MISSION.formatted(event.position().missionId()), event.position());
            }
        } catch (RuntimeException e) {
            // Un echec de diffusion ne doit jamais remonter jusqu'a
            // l'ingestion : la position est deja enregistree.
    ...
    public void onAlert(AlertTriggeredEvent event) {
        try {
            ...
            if (alert.getLevel() == AlertLevel.CRITIQUE) {
                messaging.convertAndSend(TOPIC_ALERTS_CRITICAL, payload);
            }
        } catch (RuntimeException e) {
